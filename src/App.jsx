...
import bikelaneTimeline from '../output/agents_timeline_bikelane.json';
import carbonEducationTimeline from '../output/agents_timeline_carbon_education.json';
// Bike lane scenario uses baseline timeline with a GeoJSON overlay
...
const BIKELANE_URL = new URL('./data/policies/bike_lane.geojson', import.meta.url).href;
// Carbon education points GeoJSON
const CARBON_EDU_URL = new URL('./data/policies/carbon_education_points.geojson', import.meta.url).href;
...
const colorToRgba = (c) => {
  if (!Array.isArray(c) || c.length < 3) return 'transparent';
  const [r, g, b, a = 255] = c;
...
const HIDDEN_MODES = new Set(['subway']);
...
const POLLUTION_COLORS = {
  ...
  bike: [139, 92, 246],     // purple 更清晰的骑行颜色
  walk: [16, 185, 129],     // deeper green 更高饱和度的步行颜色
};
...
const SCENARIO_DATA = {
  ...
  // Bike lane now loads dedicated timeline output generated by simulation
  bikelane: bikelaneTimeline,
  // Carbon education uses baseline timeline but overlays education points
  carbon_education: carbonEducationTimeline
};
  ...
  const SCENARIO_OPTIONS = [
    ...
    { value: 'bikelane', label: 'More Bike Lane', enabled: true },
    { value: 'carbon_education', label: 'Carbon Education', enabled: true },
  ...
  const SCENARIO_DESCRIPTIONS = {
    baseline: 'LLM-generated daily mobility without policy changes.',
    congestion_pricing: 'Dynamic toll zones discourage driving; encourages transit/walk/bike.',
    carbon_credits: 'Gamified leaderboard; scores grow with low‑carbon travel distance.',
    bikelane: 'Visual overlay of bike lanes; encourages cycling without changing routing.',
    carbon_education: 'Awareness points influence perception; shows Low‑Carbon Awareness Over Time.'
  };
...
function normalizePathWithTime(path, startTime, endTime) {
  ...
  const times = cleaned.map((p) => p.t);
  const steps = cleaned.length - 1 || 1;
  const baseStart = typeof startTime === 'number' ? startTime : (hasTime && typeof times[0] === 'number' ? times[0] : 0);
  const syntheticDuration = Math.max(60, steps * 10); // at least 60s, or 10s per segment
  const baseEnd = (typeof endTime === 'number' && endTime > baseStart) ? endTime : (baseStart + syntheticDuration);

  if (hasTime) {
    const strictlyIncreasing = times.every((t, i) => i === 0 || (typeof t === 'number' && t > times[i - 1]));
    if (strictlyIncreasing) {
      return cleaned;
    }
    const duration = Math.max(1, baseEnd - baseStart);
    return cleaned.map((p, idx) => ({ ...p, t: baseStart + (duration * idx) / steps }));
  }
...
function extractLegs(agent) {
  ...
  if (legs.length) {
    return legs.map((leg) => {
      ...
      const s2 = path[0]?.t;
      const e2 = path[path.length - 1]?.t;
      return { ...leg, mode: leg.mode ?? agent.mode, path, start_time: s2, end_time: e2 };
    });
  }
  ...
  return agent.timeline
    ...
    .map((move) => {
      const start = move.start_time ?? move.startTime ?? 0;
      const end = move.end_time ?? move.endTime ?? 0;
      const path = normalizePathWithTime(move.path ?? [], start, end);
      const s2 = path[0]?.t ?? start;
      const e2 = path[path.length - 1]?.t ?? end;
      return {
        start_time: s2,
        end_time: e2,
...
function getAgentPosition(agent, time) {
  ...
  for (const leg of agent.legs) {
    ...
    // Single-point path: render as static point throughout
    if (path.length === 1) {
      fallback = { position: startCoord, mode: leg.mode ?? agent.mode };
      continue;
    }
...
function buildReasoningLogsFromScenario(scenario) {
  ...
  // Sort by time ascending; break ties by agentId then by moveIndex to avoid one agent dominating
  logs.sort((a, b) => {
    if (a.timestamp !== b.timestamp) return a.timestamp - b.timestamp;
    const idA = String(a.agentId || '');
    const idB = String(b.agentId || '');
...
function App() {
  ...
  const [bikelaneData, setBikelaneData] = useState(null);
  const [carbonEduData, setCarbonEduData] = useState(null);
  ...
  // Sync CSS variable with bottom UI height so side panels stay clear of the controls
  useEffect(() => {
    const el = uiRef.current;
    if (!el) return undefined;

    const updateUiHeightVar = () => {
      const rect = el.getBoundingClientRect();
      if (rect && Number.isFinite(rect.height)) {
        document.documentElement.style.setProperty('--ui-height', `${Math.ceil(rect.height)}px`);
      }
    };
    ...
    if (typeof ResizeObserver === 'function') {
      resizeObserver = new ResizeObserver(updateUiHeightVar);
      resizeObserver.observe(el);
    }

    window.addEventListener('resize', updateUiHeightVar);

    return () => {
      window.removeEventListener('resize', updateUiHeightVar);
      if (resizeObserver) {
  ...
  // Load bike lane GeoJSON only when the scenario is selected
  useEffect(() => {
    let cancelled = false;
    (async () => {
      if (scenarioName !== 'bikelane') {
        setBikelaneData(null);
        return;
      }
      try {
        const res = await fetch(BIKELANE_URL);
        const gj = await res.json();
        if (!cancelled) setBikelaneData(gj);
      } catch (err) {
        // ignore read errors
      }
    })();
  ...
  // Load carbon education points only when the scenario is selected
  useEffect(() => {
    let cancelled = false;
    (async () => {
      if (scenarioName !== 'carbon_education') {
        setCarbonEduData(null);
        return;
      }
      try {
        const res = await fetch(CARBON_EDU_URL);
        const gj = await res.json();
        if (!cancelled) setCarbonEduData(gj);
      } catch (err) {
        // ignore read errors
      }
    })();
  ...
  useEffect(() => {
    ...
    // Add Mapbox official land use vector source and layer
    const addLandLayers = () => {
      // Require a valid Mapbox token for official vector tiles
      if (!mapboxgl.accessToken) return;
      try {
        // Add Mapbox Streets v8 vector source if not present
        ...
        // Insert below the first label layer for readability
        ...
        // Land use fill by class (more diverse palette + outline)
        if (!map.getLayer('landuse-fill')) {
          map.addLayer({
            ...
            paint: {
              ...
              'fill-opacity': ['interpolate', ['linear'], ['zoom'], 8, 0.25, 12, 0.35, 16, 0.45],
              'fill-outline-color': 'rgba(0,0,0,0.12)'
            }
          }, beforeId || undefined);
        }

        // Landcover fill by class (grass, scrub, crop, wood, wetland, snow) — lighter to avoid overpowering landuse
        if (!map.getLayer('landcover-fill')) {
          map.addLayer({
            ...
            paint: {
              ...
              'fill-opacity': ['interpolate', ['linear'], ['zoom'], 8, 0.18, 12, 0.25, 16, 0.3]
            }
          }, beforeId || undefined);
        }

        // Ensure landuse is drawn above landcover for better category visibility
        try {
          if (map.getLayer('landcover-fill') && map.getLayer('landuse-fill')) {
            map.moveLayer('landcover-fill', 'landuse-fill');
          }
        } catch (e) {
          // ignore reordering errors
        }
      } catch (e) {
        // Ignore failures when style/source not available
      }
    };
  ...
  const layers = useMemo(() => {
    ...
    // Bike lane overlay (only when selected)
    try {
      if (scenarioName === 'bikelane' && bikelaneData) {
        deckLayers.push(new GeoJsonLayer({
          id: 'bikelane-route',
          data: bikelaneData,
          stroked: true,
    ...
    // Carbon education points overlay (only when selected)
    try {
      if (scenarioName === 'carbon_education' && carbonEduData && Array.isArray(carbonEduData.features)) {
        const pts = carbonEduData.features
          .map((f) => {
            ...
            return { position: [lng, lat], name: f?.properties?.name || 'Education Point' };
          })
          .filter(Boolean);
        if (pts.length) {
          deckLayers.push(new ScatterplotLayer({
            id: 'carbon-edu-points',
            data: pts,
            getPosition: (d) => d.position,
    ...
    return deckLayers;
  }, [precomputed, currentTime, scenario, scenarioName, congestionZoneData, bikelaneData, carbonEduData]);
  ...
  const reasoningLogsWithColor = useMemo(() => {
    ...
    return reasoningLogs.map((entry, idx) => {
      ...
      const modeSlug = String(entry.mode || '').toLowerCase();
      return {
        ...entry,
        color: cssColor,
        key: `${entry.agentId}-${entry.timestamp}-${entry.moveIndex ?? idx}`,
        index: idx,
        displayAgentId,
        modeHidden: HIDDEN_MODES.has(modeSlug)
      };
    });
  ...
  // Precompute carbon education pass-by touches once per scenario load to avoid per-frame heavy loops
  const carbonEduTouches = useMemo(() => {
    if (!precomputed) return [];
    if (scenarioName !== 'carbon_education') return [];
    const features = Array.isArray(carbonEduData?.features) ? carbonEduData.features : [];
    if (!features.length) return [];
    const points = features.map((f, i) => {
      const coords = f?.geometry?.coordinates;
      return { idx: i, lng: Number(coords?.[0]), lat: Number(coords?.[1]) };
    }).filter((p) => Number.isFinite(p.lng) && Number.isFinite(p.lat));
    if (!points.length) return [];

    const RADIUS_M = 80; // pass-by threshold
    const CELL_SIZE_DEG = 0.001; // ~111m in lat; reduces candidate search
    const floorDiv = (x, s) => Math.floor(x / s);
    const poiGrid = new Map(); // key -> list of poi
    for (const poi of points) {
      const k = `${floorDiv(poi.lng, CELL_SIZE_DEG)}|${floorDiv(poi.lat, CELL_SIZE_DEG)}`;
      const arr = poiGrid.get(k) || [];
      arr.push(poi);
      poiGrid.set(k, arr);
    }
    const neighborKeys = (lng, lat) => {
      const i = floorDiv(lng, CELL_SIZE_DEG);
      const j = floorDiv(lat, CELL_SIZE_DEG);
      const keys = [];
    ...
    const earliest = new Map(); // key -> earliest time
    const MAX_POINTS_PER_LEG = 200; // downsample to cap checks
    for (const a of precomputed.agents) {
      ...
      for (let li = 0; li < legs.length; li++) {
        ...
        const path = Array.isArray(leg.path) ? leg.path : [];
        if (!path.length) continue;
        const step = Math.max(1, Math.ceil(path.length / MAX_POINTS_PER_LEG));
        for (let pi = 0; pi < path.length; pi += step) {
          const p = path[pi];
          const t = Number(p.t);
          if (!Number.isFinite(t)) continue;
          const lng = Number(p.lng); const lat = Number(p.lat);
          if (!Number.isFinite(lng) || !Number.isFinite(lat)) continue;
          const cands = neighborKeys(lng, lat).flatMap((k) => poiGrid.get(k) || []);
          for (const poi of cands) {
            const km = haversineKm({ lng, lat }, { lng: poi.lng, lat: poi.lat });
            const distM = km * 1000;
            if (distM <= RADIUS_M) {
              const key = `${agentId}|${li}|${poi.idx}`;
              const prev = earliest.get(key);
              if (prev == null || t < prev) earliest.set(key, t);
            }
          }
        }
      }
    }
    const touches = Array.from(earliest.entries()).map(([key, t]) => ({ key, t }));
    touches.sort((a, b) => a.t - b.t);
    return touches;
  }, [precomputed, carbonEduData, scenarioName]);

  // Awareness index up to current time using precomputed touches
  const awarenessIndex = useMemo(() => {
    if (scenarioName !== 'carbon_education') return 0;
    return carbonEduTouches.filter((x) => Number(x.t) <= currentTime).length;
  }, [carbonEduTouches, currentTime, scenarioName]);
  ...
  // Scenario legend: short, scenario-specific map overlay explanation
  const SCENARIO_LEGENDS = {
    baseline: 'No map overlay; normal mobility without policy.',
    congestion_pricing: 'Toll Zone = highlighted central polygon (red when active).',
    carbon_credits: 'Carbon Credits leaderboard only; no map overlay.',
    bikelane: 'Bike Lanes = cyan lines in the central area.',
    carbon_education: 'Education Points = green dots marking awareness sites.'
  };
  const legendText = SCENARIO_LEGENDS[scenarioName] || 'Select a scenario to view its overlay.';

  // 是否处于拥堵收费生效时段，用于图例颜色动态
  const isCongestionActive = useMemo(() => {
    try {
      const windows = (congestionZoneData?.features?.[0]?.properties?.active_hours) || [];
      if (Array.isArray(windows)) {
        for (const w of windows) {
          if (Array.isArray(w) && w.length === 2) {
            const s = Number(w[0]); const e = Number(w[1]);
            if (!Number.isNaN(s) && !Number.isNaN(e) && e > s) {
              if (currentTime >= s && currentTime <= e) return true;
            }
          }
        }
      }
    ...
    return false;
  }, [currentTime, congestionZoneData]);

  return (
    <div>
      <div ref={mapContainerRef} id="map" />

      {/* Left panel merged: scenario controls + policy info + reasoning feed */}

      <div className="reasoning-panel">
        <div className="reasoning-header">
          <div className="scenario-head">
            <div className="scenario-left">
              <h3>Low-carbon Policy Intervention Scenario</h3>
              <div className="scenario-options">
                {SCENARIO_OPTIONS.map((option) => (
                  <button
                    type="button"
                    key={option.value}
                    className={`scenario-option${scenarioName === option.value ? ' active' : ''}`}
                    disabled={!option.enabled}
                    onClick={() => setScenarioName(option.value)}
                    aria-pressed={scenarioName === option.value}
                    title={option.label}
                  >
                    <span className="scenario-label">{option.label}</span>
                    {!option.enabled && (
                      <span className="scenario-tag">Coming soon</span>
                    )}
                  </button>
                ))}
              </div>
          ...
          <div className="reasoning-separator" aria-hidden="true" />
          <h3 className="reasoning-title">LLM Reasoning</h3>
        </div>
        <div
              ...
              <div className="reasoning-item" key={entry.key} data-idx={entry.index}>
                ...
                <div className="reasoning-body">
                  <div className="reasoning-row">
                    ...
                    <span className="reasoning-time">{entry.timeLabel}</span>
                  </div>
                  {!entry.modeHidden && (
                    <div className="reasoning-mode" style={{ color: entry.color }}>Mode · {titleCaseFromMode(entry.mode)}</div>
                  )}
      ...
      <div className="topbar">
        ...
        <div className="top-banner" role="status" aria-live="polite">
          <span className="top-banner-text">
            {SCENARIO_DESCRIPTIONS[scenarioName] ?? 'Select a scenario to view its policy effects.'}
          </span>
        </div>
        <div className="top-legend" role="note">
          <div className="top-legend-row">
            <span className="top-legend-text">{legendText}</span>
            {scenarioName === 'congestion_pricing' && (
              <div className="legend-item">
                <span
                  className="legend-box"
                  style={{
                    background: colorToRgba(isCongestionActive ? [255, 90, 90, 60] : [255, 180, 90, 40]),
                    borderColor: colorToCss(isCongestionActive ? [200, 0, 0] : [180, 60, 0])
                  }}
                />
                <span className="legend-label">Toll Zone</span>
              </div>
            )}
            {scenarioName === 'bikelane' && (
              <div className="legend-item">
                <span
                  className="legend-line"
                  style={{ borderTopColor: colorToCss([64, 170, 255]) }}
                />
                <span className="legend-label">Bike Lane</span>
              </div>
            )}
            {scenarioName === 'carbon_education' && (
              <div className="legend-item">
                <span className="legend-dot legend-dot-black" style={{ background: colorToCss([16, 185, 129]) }} />
                <span className="legend-label">Education Point</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="panel">
        ...
        <div className="mode-list">
          {(() => {
            ...
            const visibleModes = ordered.filter((mode) => !HIDDEN_MODES.has(String(mode || '').toLowerCase()));
        ...
        <div className="reasoning-stats">
            ...
            (() => {
              ...
              for (const a of precomputed.agents) {
                for (const leg of a.legs || []) {
                  ...
                  if (HIDDEN_MODES.has(m)) continue;
              ...
              const modesFiltered = modesAll.filter((m) => !HIDDEN_MODES.has(String(m || '').toLowerCase()));
              const modes = [
                ...MODE_ORDER.filter((m) => modesFiltered.includes(m)),
                ...modesFiltered.filter((m) => !MODE_ORDER.includes(m))
              ];
          ...
          <div className="stats-legend">
            {(() => {
              ...
              const modesAll = Object.keys(countsByMode).filter((m) => !HIDDEN_MODES.has(String(m || '').toLowerCase()));
          ...
          {scenarioName === 'carbon_education' && (
            <div className="reasoning-stats">
              <div className="stats-header">
                <span className="stats-title">Low‑Carbon Awareness Over Time</span>
              </div>
              {(() => {
                const width = 280;
                const height = 100;
                const padL = 40, padR = 18, padT = 8, padB = 18;
                const w = width - padL - padR;
                const h = height - padT - padB;
                const bins = 24; // 24 hours
                const incByBin = new Array(bins).fill(0);
                for (const touch of carbonEduTouches) {
                  const t = Number(touch.t);
                  if (!Number.isFinite(t)) continue;
                  if (t > currentTime) continue; // only include up to current time
                  const idx = Math.max(0, Math.min(bins - 1, Math.floor(t / 3600)));
                  const key = String(touch.key || '');
                  let hcode = 0;
                  for (let i = 0; i < key.length; i++) {
                    hcode = ((hcode * 33) + key.charCodeAt(i)) >>> 0; // deterministic hash
                  }
                  const inc = (Math.abs(hcode) % 6) + 2; // 2..7 inclusive
                  incByBin[idx] += inc;
                }
                const cumInc = incByBin.slice();
                for (let i = 1; i < bins; i++) cumInc[i] += cumInc[i - 1];
                const percCounts = cumInc.map((sum) => Math.max(0, Math.min(100, 25 + sum)));
                const xOfHour = (i) => padL + ((bins > 1 ? i / (bins - 1) : 0) * w);
                const yOfPerc = (p) => padT + (1 - Math.max(0, Math.min(100, p)) / 100) * h;
                const xOfTime = (t) => padL + Math.max(0, Math.min(1, t / (24 * 3600))) * w;
                const ct = Math.min(Math.max(currentTime, 0), 24 * 3600);
                const pathParts = [];
                if (percCounts.length) {
                  pathParts.push(`M${xOfHour(0).toFixed(2)},${yOfPerc(percCounts[0]).toFixed(2)}`);
                  for (let i = 1; i < bins; i++) {
                    pathParts.push(`L${xOfHour(i).toFixed(2)},${yOfPerc(percCounts[i]).toFixed(2)}`);
                  }
                }
                return (
                  <svg className="stats-chart" style={{ width }} width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
                    <rect x={0} y={0} width={width} height={height} rx={8} ry={8} fill="rgba(255,255,255,0.75)" stroke="rgba(0,0,0,0.08)" />
                    {/* Y-axis grid at 25%, 50%, 75% */}
                    {[25, 50, 75].map((p) => (
                      <line key={`grid-${p}`} x1={padL} y1={yOfPerc(p)} x2={padL + w} y2={yOfPerc(p)} stroke="rgba(0,0,0,0.08)" />
                    ))}
                    {/* Axes */}
                    <line x1={padL} y1={padT} x2={padL} y2={padT + h} stroke="rgba(0,0,0,0.12)" />
                    <line x1={padL} y1={padT + h} x2={padL + w} y2={padT + h} stroke="rgba(0,0,0,0.12)" />
                    {/* Y-axis ticks and labels */}
                    {[0, 25, 50, 75, 100].map((p) => (
                      <g key={`ytick-${p}`}>
                        <line x1={padL} y1={yOfPerc(p)} x2={padL + 4} y2={yOfPerc(p)} stroke="rgba(0,0,0,0.25)" />
                        <text x={padL - 8} y={yOfPerc(p) + 3} fontSize={9} textAnchor="end" fill="rgba(0,0,0,0.6)">{`${p}%`}</text>
                      </g>
                    ))}
                    {pathParts.length ? (
                      <path d={pathParts.join(' ')} fill="none" stroke="rgba(16,185,129,0.9)" strokeWidth={1.2} />
                    ) : null}
                    {[0, 6, 12, 18, 24].map((hr) => {
                      const x = padL + (hr / 24) * w;
                      return (
                        <g key={`tick-awareness-${hr}`}>
                          <line x1={x} y1={padT + h} x2={x} y2={padT + h + 4} stroke="rgba(0,0,0,0.25)" />
                          <text x={x} y={padT + h + 12} fontSize={8} textAnchor="middle" fill="rgba(0,0,0,0.6)">{`${hr}h`}</text>
                        </g>
                      );
                    })}
                    <line x1={xOfTime(ct)} y1={padT} x2={xOfTime(ct)} y2={padT + h} stroke="rgba(37,99,235,0.9)" strokeDasharray="3 3" />
